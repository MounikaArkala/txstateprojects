...
def unique(lst):
    """
    Function rewritten by Luke P. to be around 5 times more time-efficient.  Returns unique items in a list.
    """
    return list(set(lst))
...
def choose_attribute(data, attributes, target_attr, fitness):
    ...
    if fitness == 'RANDOM':
        #get a list of attributes other than target_attr
        attribs = [i for i in attributes if i != target_attr]
        return random.choice(attribs)
