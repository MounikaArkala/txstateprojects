...
def start( exedir='', port=57110, inputs=2, outputs=2, samplerate=44100, verbose=0,
    ...
    if sys.platform == 'win32' :
        exe  = 'scsynth.exe'
        if exedir == '' : exedir = 'C:\Program Files\PsyCollider' 
    elif os.uname()[0] == 'Linux' :
        exe = 'scsynth'
        if exedir == '' : exedir = '/usr/bin'
        full = os.path.join(exedir, exe)
        if not os.path.isfile(full) :
            exedir = '/usr/local/bin'
            full = os.path.join(exedir, exe)
            if not os.path.isfile(full) :
                print "ERROR : scsynth not found in your system, please install or compile supercollider"
...
##def status() : server.sendMsg('/status')
...
##        print path, 'sound exists, loading...'
...
class Synth(object) :
    ...
    def __init__(self, stringdefname='', nodeID=-1, addAction=1, addTargetID=0) : #, args=[] ) :
        ...
        if Synth.loadedSynthdefs.count(self.defpath) == 0 : # check if already loaded
            if os.path.isfile( self.defpath ) :
##                print 'synthdef %s file exists ...' % self.defpath
                server.sendMsg('/d_load', self.defpath)
                time.sleep(0.5) #wait till loaded
                #sndLoader.server.receive('/done', '/fail') # wait till loaded.
...
##    def printControl(self, control, n) :
##        """ together with server verbose 1 on server prints the control range to python console.
##        Useful for introspecting bytecoded synthdefs?
##        """
##        server.sendMsg('/s_getn',  self.nodeID, control, n)
