    ...
    def add(self, data):
        ...
        #otherwise we just add it to the base case.
            ...
            if existingval[0][-1] == data[-1]: #assume class is last variable.
                ...
                attrs = data.domain # get rid of class.
                #print "attrs:", attrs
                targets = []
                for item in attrs:
...
def makeTree(data):
    ...
    #print head
    return head
    #print dir(data[0].domain[0])
    #items = 
#print dir(data)

#DON'T need to implement this, just implement height!
"""
def printTree(startNode, level=0):
    print "SPLIT ON ", startNode.splitvar
    for i in startNode.splitvar.values:
        print "\t"*level, i, "->",
...
def height(startNode, current=0, max=0):
    if current > max:
        max = current
    terminal = False
    for tree in startNode.subtrees.values():
        if type(tree) == type(startNode):
            max = height(tree, current+1, max=max)
        elif tree != None:
...
#TODO: make a function that can check a value for correctness (may have to add "default value" function)
#and then cross-validate it.
head = makeTree(data)
print height(head)
classification_accuracy = 0
for i in range(len(data)):
    if data[i] == getClass(head, data[i]):
        classification_accuracy += 1
classification_accuracy /= float(len(data))
