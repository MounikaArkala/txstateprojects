...
alphasurf.fill((255,255,255), (0,0,width,height))
...
while not exit:
    ...
    if end_event and drawevent:
        #our temp alpha surface should be blitted to the actual canvas now.
        ...
        if mousedown:
            if tool == "eyedropper":
                ...
                for pos in drawlocs:
                    ...
                    tempsurf = pygame.Surface((radius*2, radius*2), flags=SRCALPHA)
                    tempsurf.fill((0,0,0,0))
                    tempsurf.set_alpha(trans)
                    tempsurf.convert_alpha()
                    pygame.draw.circle(tempsurf, color, (radius, radius), radius)
                    canvas.blit(tempsurf, (pos[0][0]-radius, pos[0][1]-radius))
