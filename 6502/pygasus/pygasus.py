...
for i in xrange(30*2):
	tilePrefetch[i]=[0]*32*2
	for j in xrange(32*2):
		tilePrefetch[i][j]=pygame.Surface((8,8))
pygame.init()
...
def tpFone2(x,y):
    ...
    for yo in xrange(8):
        ...
        for xo in xrange(8):
            ...
            tilePrefetch[y][x].set_at((xo,yo),finalColor)
def tpFone(x,y):
    global tilePrefetch
    if ppuNameTableAdr==0x2000:
        nta=0x2000
	...
	for y in xrange(240):
		...
		for x in xrange(64):
			...
			for x2 in xrange(8):
				...
				color=ord(ppuRAM[cpos])&0x3f
				finalColor=pallete[color]
				tilePrefetch[ydiv][x].set_at((x2,y7),finalColor)
	for y in xrange(240):
		addr0=nta2+((y>>3)<<5)
		...
		for x in xrange(32):
			...
			for x2 in xrange(8):
				...
				color=ord(ppuRAM[cpos])&0x3f
				finalColor=pallete[color]
				tilePrefetch[ydiv][x+32].set_at((x2,y7),finalColor)        
...
def ppuDoScanline(n):
	...
	if ppuShowBackgrnd:
		...
		if n%8==0:
			for x in xrange(32):
                            sx=(sx+1)%64
                            hScreen.blit(tilePrefetch[sy][sx],((x<<3)-hor0,n-ver0))
	if ppuShowSprites_:
		sprPerScln=0
		...
		for j in xrange(64):
			...
			if ppuSprSizeValue:
				...
				for x in xrange(8):
					...
					finalColor=pallete[color]
					if indx==0 and hScreen.get_at((xpos+x,n))==bgcolor:
						ppuStatusRegstr|=0x40
					if finalColor!=bgcolor:
						hScreen.set_at((xpos+x,n),finalColor)
#					hScreen.set_at((xpos+x,n),[255]*3)
			else:
				ypos=ord(SPRRAM[(i2)])
...
def pGetMem(adr):
	...
	adr&=0xffff
	if adr>=0x8000:
		return ord(mmcRead[mMapper](adr-0x8000))
	elif adr<0x2000:
...
def pExec(cycles):
	global ticks,opcode,PC, ppuStatusRegstr,PS, keys, hScreen,ppuVRAMAdrValue,addr0,	tilesModified
	tscanline=0
	nscanline=0
	cntr=0
	vbwait=0
	tt=0
	while cycles==-1 or cycles>0:
		cntr+=1
		if cntr>1000:
			cntr=0
			pygame.event.poll()
...
#					tilesModified=0
		...
		if tilesModified and tt>500000:
			if tilesModified:
                            tt=0
                            tpF()
                            tilesModified=0			
...
f=file("dbg/_SPRRAM","w")
f.write("".join(SPRRAM))
f.close()

f=file("dbg/_nesRAM","w")
f.write("".join(nesRAM))
f.close()

for i in enumerate(PRGbks):
    f=file("dbg/_PRG"+str(i[0]),"w")
    f.write("".join(i[1]))
    f.close()

for i in enumerate(CHRbks):
    f=file("dbg/_CHR"+str(i[0]),"w")
    f.write("".join(i[1]))
    f.close()

for i in enumerate(tilePrefetch):
	for j in enumerate(i[1]):
		pygame.image.save(j[1],"dbg/tile"+str(i[0])+"x"+str(j[0])+".jpg")
